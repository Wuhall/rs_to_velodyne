...
int main(int argc, char **argv) {
    ...
    node = std::make_shared<rclcpp::Node>("rs_converter");
    ...
    if (argc < 3) {
        ...
        if (std::strcmp("XYZI", argv[1]) == 0) {
            subRobosensePC = node->create_subscription<sensor_msgs::msg::PointCloud2>(
                "/rslidar_points", 1, rsHandler_XYZI);
        } else if (std::strcmp("XYZIRT", argv[1]) == 0) {
            subRobosensePC = node->create_subscription<sensor_msgs::msg::PointCloud2>(
                "/rslidar_points", 1, rsHandler_XYZIRT);
        } else {
            RCLCPP_ERROR(node->get_logger(), "Unsupported input pointcloud type: %s", argv[1]);
            RCLCPP_ERROR(node->get_logger(), "Currently only support XYZI and XYZIRT.");
            return 1;
        }
    }
